...
# COMBINED LOG + RANGER AUDIT LOGS

cat $1/aws-ec2-and-idbroker-role-trust-policy.json |  sed s/AWS_ACCOUNT_ID/$AWS_ACCOUNT_ID/g | sed s/IDBROKER_ROLE/$IDBROKER_ROLE/g  > $1/tmp
aws iam create-role --role-name $2-log-role --assume-role-policy-document file://$1/tmp
sleep $sleep_duration 
...
aws iam attach-role-policy --role-name $2-log-role --policy-arn arn:aws:iam::$AWS_ACCOUNT_ID:policy/$2-ranger-audit-policy-s3access
sleep $sleep_duration 

aws iam attach-role-policy --role-name $2-log-role --policy-arn arn:aws:iam::$AWS_ACCOUNT_ID:policy/$2-dynamodb-policy
sleep $sleep_duration 
